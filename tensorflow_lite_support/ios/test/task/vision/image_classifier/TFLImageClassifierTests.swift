...
class ImageClassifierTests: XCTestCase {
  ...
  func testInferenceOnMLImageWithUIImage() throws {
    ...
    let imageClassifierOptions = try XCTUnwrap(ImageClassifierOptions(modelPath: modelPath))

    let imageClassifier =
      try XCTUnwrap(ImageClassifier.imageClassifier(options: imageClassifierOptions))

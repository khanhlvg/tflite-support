...
+ (BOOL)createFromWavFileWithPath:(NSString *)filePath error:(NSError **)error {
   
  // Specifying AVAudioPCMFormatFloat32 since task library needs samples in float32 format.
  AVAudioFormat *processinFormat =
      [[AVAudioFormat alloc] initWithCommonFormat:AVAudioPCMFormatFloat32
                                       sampleRate:self.audioFormat.sampleRate
                                         channels:(AVAudioChannelCount)self.audioFormat.channelCount
                                      interleaved:YES];
  
  AVAudioPCMBuffer *buffer = [AVAudioPCMBuffer loadPCMBufferFromFileWithURL:[NSURL fileURLWithPath:filePath] processingFormat: error:error];
  return 
}
